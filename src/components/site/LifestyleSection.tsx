...
export function LifestyleSection({
  ...
  const open = openSlug
    ? (allProducts.find((p) => p.slug === openSlug) as Product | undefined)
    : null;
    ...
    <section className="bg-cream">
      <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 overflow-hidden rounded-3xl shadow-xl lg:grid-cols-2">
          {/* LEFT — emerald with circular illustration */}
          <div className="relative flex min-h-[420px] flex-col justify-between bg-emerald-deep p-10 text-cream lg:p-14">
            ...
            <div className="relative mt-10">
              ...
              <h2 className="mt-3 font-serif text-3xl leading-tight md:text-4xl">
                Nature Meets <span className="italic text-gold">Modern Science.</span>
              </h2>
              <p className="mt-4 max-w-md text-sm leading-relaxed text-cream/80">
                Every formula begins with botanicals trusted for generations, then is refined with
                modern nutritional research. We make wellness that fits everyday life — honest,
                effective, and crafted with intention.
              </p>
            </div>
          </div>

          {/* RIGHT — 2x3 mini tiles */}
          <div className="bg-card p-6 sm:p-8 lg:p-10">
            <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-primary">
              Quick Browse
            </p>
            <h3 className="mt-1 font-serif text-2xl text-foreground md:text-3xl">
              Top picks across wellness
            </h3>
              ...
              {TILE_SLUGS.map(({ slug, icon: Icon, tint }) => {
                ...
                return (
                  <button
                    ...
                    className={`group flex items-start gap-3 rounded-2xl border p-3 text-left transition-all hover:-translate-y-0.5 hover:shadow-md sm:p-4 ${
                      active
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="mt-0.5 block truncate font-serif text-sm leading-tight text-foreground sm:text-base">
                        {p.name.replace("FSG ", "")}
                      </span>
                    </span>
                    <ArrowRight
        ...
        {open && (
          <ProductDetailPanel product={open} onClose={() => setOpenSlug(null)} />
        )}
      </div>
    </section>
  );
