...
type Slide = {
  ...
  photo: string;
  focus?: string;
  tone: { from: string; via?: string; accent: string }; // rgba for overlay tint
};

// Unsplash CDN — verified stable photo IDs of real people in lifestyle settings.
const u = (id: string) =>
      ...
      <div className="relative mx-auto aspect-[4/5] w-full max-w-md overflow-hidden rounded-3xl bg-emerald-deep shadow-2xl ring-1 ring-border/40">
        {slides.map((s, i) => (
          ...
          >
            ...
            />
            {/* Vertical gradient for headline + bottom info legibility */}
            <div
        ...
        <div className="absolute inset-x-0 top-0 p-6 sm:p-7">
          ...
          >
            ...
            <span className="text-[10px] font-semibold uppercase tracking-[0.22em] text-cream">
              {current.eyebrow}
            </span>
          </div>
          <h2
        ...
        {/* Bottom info bar — always visible, product card(s) inside */}
        <div className="absolute inset-x-3 bottom-3 sm:inset-x-4 sm:bottom-4">
          ...
          >
            {currentProducts.map((p) => (
              <ProductCardOverlay key={p.slug} product={p} />
            ))}
          </div>
        </div>
          ...
          {slides.map((_, i) => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all duration-500 ${
                i === index ? "w-8 bg-primary" : "w-2 bg-foreground/25 hover:bg-foreground/50"
              }`}
            />
          ))}
        </div>
